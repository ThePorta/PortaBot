		...
		for tokenName, tokenAddr := range chain.SupportedTokens {
			for _, account := range accounts {
				...
				if allowanceAmount.Cmp(big.NewInt(0)) > 0 {
					...
					msgConfig := tgbotapi.NewMessage(chatId, fmt.Sprintf("Security Warning⚠️⚠️\nyour account %s approve the malicious address %s %s %s on %s. Please open %ssubmitTx?uuid=%s to revoke", account, maliciousAddress, allowanceAmount.String(), tokenName, chain.ChainName, config.URL, uuidStr))
